...
public class MonthlyEvent extends RepeatingEvent {
	...
	/**
	 * Constructs a new repeating monthly event.
	 * 
	 ...
	 */
	public MonthlyEvent(String eName, LocalDate fOccur, int rep, LocalTime sTime, LocalTime eTime) {
	...
	public boolean isOnDay(LocalDate when) {
		...
		LocalDate lastOccurrence = firstOccurrence.plusMonths(getRepetitions());

		// checks
		if (when.equals(firstOccurrence)) {
			return true;
		}
		if (when.getDayOfMonth() != firstOccurrence.getDayOfMonth()) {
			return false;
	 ...
	 *         times the event repeats and it's first occurrence, monthly.
	 */
	@Override
